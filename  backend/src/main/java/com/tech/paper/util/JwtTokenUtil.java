...
public class JwtTokenUtil {
    ...
    public static Claims parseJWT(String jsonWebToken, String base64Security) {
        try {
            ...
            throw new ServiceException(MessageStatus.TOKEN_EXPIRED,"token expired");
        } catch (Exception e){
            log.error("===== token解析异常 =====", e);
            throw new ServiceException(MessageStatus.FAILURE,"token parsing exception");
        }
    }
    /**
     * 构建jwt
    ...
    public static String createJWT(String userId, String username, String role, Audience audience) {
        try {
            ...
            //userId是重要信息，进行加密下
            String encryId = Base64.getEncoder().encodeToString(userId.getBytes("utf-8"));
            //添加构成JWT的参数
            ...
            log.error("签名失败", e);
            throw new ServiceException(MessageStatus.FAILURE,"签名失败");
        }
    }
    /**
     * 从token中获取用户名
